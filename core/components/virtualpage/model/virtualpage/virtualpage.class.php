...
class virtualpage
{
    ...
    {
        $cache_prefix = $this->modx->getOption('cache_prefix');
        $cacheKey = $this->modx->getOption(xPDO::OPT_CACHE_PATH) . 'default/' . $this->namespace . '/'.$cache_prefix.'fastroute/' . sha1(serialize($routes));
